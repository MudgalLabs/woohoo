...
- [x] **Extension (content modals + popup)** — theme matching (`content/lib/theme.ts` + `useTheme` hook, auto dark/light against Reddit's theme class), SaveModal auto-close + bottom-right `Toast` via `toastMount`, standalone ext `DateTimePicker`, popup `account-stats` + Open dashboard button. Plan: `~/.claude/plans/wobbly-dazzling-feigenbaum.md`.
...
- [x] Save-rule enforcement — deterministic comment routing: peer-authored comments always go to the author's own Woohoo (ignoring ancestors); founder-authored comments attach to a saved peer ancestor when one exists, else to the founder's own Woohoo. Founder username scraped from `#user-drawer-content` (new `ext/src/content/reddit/founder.ts` with `normalizeRedditUsername` helper; strips `u/` prefixes) and overridable via a popup field. SaveModal checkbox removed; replaced with a read-only "Saving to u/X's Woohoo" preview. Unsave flow restored in the saved-state modal (was dropped in the rewrite). `forceNewWoohoo` dropped from shared API types. Detailed brief in "Deferred / backlog" below (kept as reference).
